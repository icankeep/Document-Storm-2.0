...
public class TextToPDFService {
	...
	public void createPDFFromText(PDDocument doc, Reader text) throws IOException {
		try {
				...
				while (lineIndex < lineWords.length) {
					...
					do {
						...
						if (lineIndex < lineWords.length) {
							...
							lengthIfUsingNextWord = (font.getStringWidth(lineWithNextWord) / FONTSCALE) * fontSize;
						}
			...
			// If the input text was the empty string, then the above while loop will have
			// short-circuited
			// and we will not have added any PDPages to the document.
			// So in order to make the resultant PDF document readable by Adobe Reader etc,
			// we'll add an empty page.
			if (textIsEmpty) {
				doc.addPage(page);
			}
	...
	public static void main(String[] args) throws IOException {
		...
		TextToPDFService app = new TextToPDFService();
